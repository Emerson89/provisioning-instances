...
| <a name="input_availability_zone"></a> [availability\_zone](#input\_availability\_zone) | AZ to start the instance in | `list(any)` | <pre>[<br>  "us-east-1a",<br>  "us-east-1b",<br>  "us-east-1c"<br>]</pre> | no |
...
| <a name="input_eip"></a> [eip](#input\_eip) | Enabled eip | `string` | `"false"` | no |
...
| <a name="input_ingress"></a> [ingress](#input\_ingress) | Ingress rules security group | `map(any)` | `{}` | no |
...
| <a name="input_key_name"></a> [key\_name](#input\_key\_name) | Key name of the Key Pair to use for the instance; which can be managed using the `aws_key_pair` resource | `string` | `"key-pem"` | no |
...
| <a name="input_values"></a> [values](#input\_values) | Values datasource ami | `string` | `"ubuntu/*"` | no |
